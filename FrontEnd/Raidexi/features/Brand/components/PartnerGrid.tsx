...
const PartnerGrid: React.FC = () => {
  return (
    <section className="w-full border-t border-border-subtle font-sans">
      <div className="grid grid-cols-2 mx-auto border-l border-border-subtle md:grid-cols-3 lg:grid-cols-5 bg-surface-dark">
        {PARTNER_ITEMS.map((item, index) => {
          if ('type' in item && item.type === 'join') {
            return (
              <div 
                key={`join-${index}`}
                className="aspect-4/3 flex items-center justify-center border-r border-b border-border-subtle p-8 group hover:bg-surface-hover transition-all duration-300 cursor-pointer shadow-sm"
              >
          ...
          if ('src' in item) {
             return (
              <div 
                key={`partner-${index}`}
                className="aspect-4/3 flex items-center justify-center border-r border-b border-border-subtle p-8 group hover:bg-surface-hover hover:shadow-sm transition-all duration-300"
              >
                <img
