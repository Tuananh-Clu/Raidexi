  ...
  return (
    <section className="px-6 py-24 border-b border-border-subtle">
      <div className="mx-auto max-w-7xl">
        <h2 className="mb-16 text-3xl font-medium text-center text-[#0f172a]">Process Logic</h2>
        <div className="relative grid items-stretch gap-4 md:grid-cols-4">
          ...
          {steps.map((step) => (
            <div key={step.id} className="z-10 flex flex-col items-center justify-center p-6 text-center transition-colors border cursor-default bg-[#f8fafc] border-border-subtle min-h-45 hover:border-primary group">
              ...
              <h4 className="mb-2 font-medium text-[#0f172a]">{step.title}</h4>
              <p className="text-xs text-[#64748b] font-mono uppercase">{step.sub}</p>
            </div>
          ))}
        </div>
      </div>
